...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/lessons.mjs onboard --file '<path>'
```

Report the output. Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/lessons.mjs build` if any were added. Delete the progress file and end.
...
- **Approve batch** — import all lessons in this batch:
  ...
  ```bash
  node ${CLAUDE_PLUGIN_ROOT}/scripts/lessons.mjs onboard --file '<path>' --from <startIndex> --count <batchSize>
  ```
...
- **Approve** — `node ${CLAUDE_PLUGIN_ROOT}/scripts/lessons.mjs add --json '<json-object>'`
