...
| `type`               | `hint` (informational), `guard` (blocks destructive action), `protocol` (session-start once), `directive` (session-start always), `reminder` (PostToolUse — fires after tool result matches `outputPatterns`)                                                                                                                                                                                                                                                                            |
| `toolNames`          | **Required.** Use exact casing: `Bash`, `Read`, `Edit`, `Write`, `Glob`, `Grep`, `Agent`, `TodoWrite`, `WebFetch`, `WebSearch`. "TypeScript", "SQL", "Git", "npm", "curl", "Vercel" are NOT valid — map them to `Bash`, `Edit`, `Write` as appropriate. **When targeting `Read`/`Edit`/`Write`/`Glob`/`Grep`, you MUST also add the Serena equivalents (see "Serena tool equivalents" below) — the agent often edits via Serena, so a native-only trigger silently misses those calls.** |
...
**Serena tool equivalents:**

The lessons PreToolUse inject hook fires on Serena MCP tools as well as native ones. Serena is the preferred path for code work, so a lesson that only lists native `Edit`/`Write`/`Read` will silently miss the Serena calls the agent actually makes. **Whenever a candidate targets a code tool, add the matching Serena equivalents to `toolNames` alongside the native name** (all prefixed `mcp__plugin_serena_serena__`):

| Native  | Serena equivalents to also include                                                      |
| ------- | --------------------------------------------------------------------------------------- |
| `Read`  | `read_file`, `find_symbol`, `get_symbols_overview`                                      |
| `Edit`  | `replace_symbol_body`, `replace_content`, `insert_after_symbol`, `insert_before_symbol` |
| `Write` | `create_text_file`, `insert_after_symbol`, `insert_before_symbol`                       |
| `Glob`  | `find_file`                                                                             |
| `Grep`  | `search_for_pattern`                                                                    |

Caveat: `pathPatterns` do not yet match Serena calls (the hook reads native `file_path`, not Serena's `relative_path`). For code lessons that need Serena coverage, prefer `toolNames` + `scope` over `pathPatterns`, or accept that pathPatterns-gated lessons only fire on native edits.
