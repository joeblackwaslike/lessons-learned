...
type                TEXT NOT NULL DEFAULT 'hint'
                    -- directive | guard | hint | protocol | reminder (see Type taxonomy below)
...
outputPatterns      TEXT NOT NULL DEFAULT '[]'   -- JSON array of regex strings, matched against tool_response (reminder type)
...
| `reminder`  | Inject as blockquote PostToolUse when `outputPatterns` matches `tool_response` |
