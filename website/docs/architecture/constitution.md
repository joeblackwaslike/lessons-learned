...
description: Stable principles governing lesson quality and injection philosophy for the lessons-learned plugin.
---
...
**Blocking is exceptional.** `type: 'guard'` on a lesson denies the tool call entirely. This is reserved for commands with known data-loss or irreversible consequences where the lesson _is_ the prevention. Use it rarely and deliberately.
...
- **Global** (`scope: null`): injected in any project. Appropriate for tool-level mistakes that apply universally (git, pytest, TypeScript, etc.).
- **Project-specific** (`scope: "<project-id>"`): injected only when the hook's `cwd` derives to a matching project ID. Appropriate for mistakes tied to a project's specific conventions, stack, or architecture.
...
**Tier 2 (heuristic):** Sliding-window pattern detection over session JSONL files, looking for error→correction sequences in tool results and assistant messages. Lower fidelity — requires human review via `/lessons:review` or `promote --ids <id>` before entering the store.

**Tier 3 (structural) and Tier 4 (deep scan):** Lexical pattern detection over semantic windows, and LLM-assisted extraction (requires `ANTHROPIC_API_KEY`), respectively — see [Project Overview](project.md#scanning-tiers) for detail.
...
| Content                                    | Where                            |
| ------------------------------------------ | -------------------------------- |
| Repeatable mistake with a specific trigger | Lesson store (`data/lessons.db`) |
| One-time setup note or project convention  | `CLAUDE.md`                      |
| Reasoning reminder with no trigger         | `type: 'protocol'` lesson        |
