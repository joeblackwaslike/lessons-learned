---
sidebar_position: 7
title: Eval Scenario Writing Guide
description: How to write effective evaluation scenarios that reliably reproduce failure modes and validate lesson injection.
---
...
:::note SessionStart hooks in `--print` mode
`SessionStart` hooks **do** fire in `--print` mode as long as CC is invoked with `cwd` set to the workspace directory (so it can find `.claude/settings.json`). The eval provider already does this. If a SessionStart hook appears not to fire, debug the hook command itself — not the invocation mode.
:::
