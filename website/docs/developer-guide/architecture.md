...
sidebar_position: 1
title: Architecture
description: Component map, data flow diagrams, and design decisions for the lessons-learned plugin.
---
...
After promotion, `lessons build` compiles the database → `lesson-manifest.json`. The manifest is a pre-compiled, pre-indexed version of the store optimized for fast hook-time loading.
