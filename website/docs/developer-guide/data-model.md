...
```jsonc
{
  ...
  // directive | guard | hint | protocol | reminder — the authoritative injection behavior
  "type": "guard",
  ...
  // Regex patterns matched against tool_response (stdout). Used with type: "reminder"
  "outputPatterns": [],
...
| `outputPatterns`    | Valid regex — matched against `tool_response` (PostToolUse). Only used with `type: "reminder"` |
