...
description: JSON Schema definitions for lessons.json, lesson-manifest.json, and config.json data files.
---
...
`lesson-manifest.json` is generated by `node scripts/lessons.mjs build`. Edit `lessons.json` (or use the CLI) and rebuild instead.
:::
