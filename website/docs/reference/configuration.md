...
description: Complete reference for all fields in data/config.json, including environment variable overrides and rebuild requirements.
---
...
Lessons with `confidence` below this value are excluded from the manifest at `lessons build` time. They still exist in the database.
