---
sidebar_position: 2
title: 'PRD-002: /lessons:review Command'
description: Design specification for the unified lesson review command with LLM-assisted filtering and conversational promotion workflow.
---
...
The current Jaccard tokenization approach produces false positives as the lesson store grows. MinHash generates a compact signature per lesson and enables accurate set-similarity estimation with sub-linear lookup. This is worth evaluating as a replacement at ~200+ lessons. Deferred, but the schema is designed to accommodate it (nullable embeddings column).
...
Only records the user explicitly interacts with are written on any given session. Untouched candidates remain as `candidate` and reappear in the next review.
...
Then rebuilds `lesson-cache.json`.
...
The only runtime requirement is Node.js ≥ 22.5 (for `node:sqlite`).
...
1. `scripts/migrate-db.mjs` — DB has correct active + candidate records; `.bak` files present
2. `/lessons:review` — scan runs, aggregate runs, LLM review writes `data/review-sessions/<ulid>.json`
3. Confirm promotion — selected records show `status: 'active'` in DB; manifest rebuilt
4. Untouched candidates — verify they remain `status: 'candidate'` in DB
5. Re-run `/lessons:review` — untouched candidates reappear; promoted records absent
6. Empty state — "No candidates found. Run `lessons scan` to discover new ones."
