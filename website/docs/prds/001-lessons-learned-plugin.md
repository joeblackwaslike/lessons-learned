---
sidebar_position: 1
title: 'PRD-001: Lessons-Learned Plugin'
description: Design specification for automatic mistake capture and proactive lesson injection in AI coding agents.
---
...
3. **Decision gate**: If compliance > 70%, Tier 1 (structured) is the primary scanner. If < 40%, Tier 2 (heuristic) is primary.

### Phase 1: Harvest & Schema Validation

4. Build `scripts/scan.mjs` and `scripts/scanner/`
5. Run full scan against all existing sessions — collect ALL candidates
6. Finalize the lesson schema based on real data
7. Curate seed lessons from the best candidates

### Phase 2: Plugin Skeleton + Hook

8. Create `.plugin/plugin.json`, `package.json`, `schemas/`
9. Implement `scripts/build-manifest.mjs`
10. Implement hook lib modules and `hooks/pretooluse-lesson-inject.mjs`
11. **Test**: Install plugin, verify lessons inject correctly

### Phase 3: Store Operations + Commands + CLI Intelligence

12. Implement `scripts/add-lesson.mjs` (ULID generation, slug with random suffix)
13. Auto-rebuild manifest on lesson store changes
14. Create `commands/scan-lessons.md` and `commands/add-lesson.md`
15. Begin aggregating dense tool clusters into CLI intelligence skills

### Phase 4: Automation + Cross-Agent

16. Add SessionStart background scan trigger
17. Add `--auto` mode for heuristic-only classification
18. Refactor core logic into agent-agnostic `core/` module
19. Document adapter interface for Codex/Gemini
