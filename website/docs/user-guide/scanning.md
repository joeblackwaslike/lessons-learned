...
description: How the background scanner discovers lesson candidates from session logs across four tiers.
---
...
```mermaid
flowchart LR
    A["JSONL\nsession files"] --> T1["Tier 1: Structured\nParse #lesson tags\n(high fidelity)"]
    A --> T2["Tier 2: Heuristic\nError→correction\npattern detection"]
    A --> T3["Tier 3: Structural\nInsight announcements\n(manual review)"]
    A --> T4["Tier 4: LLM\nFull-session analysis\n(ANTHROPIC_API_KEY req.)"]
    T1 --> D["Dedup by\ncontentHash"]
    T2 --> D
    T3 --> W["pending_semantic_windows\n(DB)"]
    T4 --> D
    D --> DB["lessons.db\ncandidates"]
    DB --> R["lessons review\n→ promote"]
    R --> M["lesson-manifest.json"]
```
...
:::warning Tier 4 requires ANTHROPIC_API_KEY
