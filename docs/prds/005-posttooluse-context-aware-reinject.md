...
Location options (open question for brainstorm):

- `$TMPDIR/lessons-<sessionHash>-reinject.json` — current convention for dedup/seen state
- `/var/run/lessons-<sessionHash>-reinject.json` — more stable across temp cleans, may require write permissions
...
4. Run `node --test 'tests/unit/**/*.test.mjs'` — 175 tests must pass
