...
| `requires`           | Lesson excluded from manifest unless artifact is installed. Single object or array (OR logic). Shapes: `{"type":"plugin","name":"serena"}`, `{"type":"mcp-server","name":"github"}`, `{"type":"skill","name":"superpowers:brainstorming"}`, `{"type":"cli","name":"no-mistakes"}`. Use an array when a tool ships as both a plugin and a standalone MCP server. |
