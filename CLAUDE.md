...
| `requires`           | Lesson excluded from manifest unless artifact is installed. Single object or array (OR logic). Shapes: `{"type":"plugin","name":"serena"}`, `{"type":"mcp-server","name":"github"}`, `{"type":"skill","name":"superpowers:brainstorming"}`. Use an array when a tool ships as both a plugin and a standalone MCP server. |
...
`summary`, `problem`, `solution`, `type`, `scope`, `toolNames`, `commandPatterns`, `commandMatchTarget`, `pathPatterns`, `priority`, `confidence`, `tags`, `requires`, `duplicatedBy`
